...
static void segv_handler(int sig, siginfo_t* info, void* ctx)
{
	...
	if (sig == SIGBUS)
		valid = 1;
#endif
	// DIAG: log signal details to both stderr and a file on the target.
	// All functions used here are async-signal-safe.
	{
		char diagbuf[256];
		...
		if (n > 0) {
			write(2, diagbuf, n);
			// Also append to a file in case stderr is lost through pipes.
