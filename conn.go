...
// rpcDiagCRC enables appending a CRC32 checksum (IEEE polynomial, little-endian)
// after every sent RPC message payload. The executor verifies this checksum when
// compiled with -DSYZ_RPC_DIAG=1. A CRC mismatch definitively proves network or
// kernel TCP corruption (as opposed to a host serialization bug or executor memory
// corruption). IMPORTANT: this flag must be synchronized with SYZ_RPC_DIAG in
// executor/conn.h — both sides must agree, otherwise the executor will read
// CRC bytes as part of the next message's size prefix, desynchronizing the stream.
const rpcDiagCRC = false
...
func Send[T sendMsg](c *Conn, msg T) error {
	...
	if err == nil && rpcDiagCRC {
		// Append CRC32 (IEEE polynomial, little-endian) of the payload
		// (everything after the 4-byte size prefix). The executor reads
		// this when compiled with SYZ_RPC_DIAG=1 and verifies integrity.
		payload := data[flatbuffers.SizeUint32:]
		checksum := crc32.ChecksumIEEE(payload)
		var crcBuf [4]byte
		binary.LittleEndian.PutUint32(crcBuf[:], checksum)
		_, err = c.conn.Write(crcBuf[:])
	}
	c.builder.Reset()
