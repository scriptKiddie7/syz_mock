...
private:
	...
	uint64 attempts_ = 0;
	uint64 total_failures_ = 0; // persistent counter — NOT reset by new requests
	uint64 freshness_ = 0;
	...
	void Restart(bool is_failure = true)
	{
		debug("proc %d: restarting subprocess, current state %u attempts %llu total_failures %llu is_failure=%d\n",
		      id_, state_, attempts_, total_failures_, is_failure);
		int status = process_->KillAndWait();
		process_.reset();
		debug("proc %d: subprocess exit status %d\n", id_, status);
		if (is_failure) {
			total_failures_++;
			// Persistent failure limit: unlike attempts_ (which resets on new requests),
			// total_failures_ only resets on successful execution. If the child keeps
			// failing (e.g. pthread_create EPERM from per-user process limit exhaustion),
			// exit the runner cleanly so the manager restarts the instance.
			// NOTE: Do NOT use failmsg/SYZFAIL here — that would trigger crash reproduction.
			// A clean doexit(0) causes "lost connection" which the manager handles as a
			// routine instance restart.
			if (total_failures_ > 50) {
				debug("proc %d: %llu consecutive child failures, restarting runner\n",
				      id_, total_failures_);
				fprintf(stderr, "runner: too many consecutive child failures (%llu), "
					"restarting to clear process table\n", total_failures_);
				doexit(0);
			}
		} else {
			total_failures_ = 0;
	...
	void Start()
	{
		...
		// DIAGNOSTIC: print all source→target fd mappings before posix_spawn
		debug("proc %d: fd mappings: req_pipe[0]=%d→0, resp_pipe[1]=%d→1, stdout_pipe[1]=%d→2, "
		      "req_shmem=%d→%d, resp_shmem=%d→%d, max_signal=%d→%d, cover_filter=%d→%d\n",
		      id_, req_pipe[0], resp_pipe[1], stdout_pipe[1],
		      req_shmem_.FD(), kInFd, resp_shmem_.FD(), kOutFd,
		      max_signal_fd_, kMaxSignalFd, cover_filter_fd_, kCoverFilterFd);
	...
	void HandleCompletion(uint32 status, bool hanged = false)
	{
		...
		debug("proc %d: sending ExecResult to manager: req_id=%llu status=%u num_calls=%u data_size=%zu\n",
		      id_, static_cast<uint64>(msg_->id), status, num_calls, data.size());
...
#if !SYZ_EXECUTOR_USES_FORK_SERVER
		// Normal lifecycle end in non-fork-server mode: child exited after
		// execution. This is not a failure — reset total_failures_.
		if (process_)
			Restart(/*is_failure=*/false);
#endif
	}
	...
	bool ReadResponse(bool out_of_requests)
	{
		...
		// DIAGNOSTIC: hex-dump every value read from resp_pipe
		debug("proc %d: resp_pipe read: status=0x%08x (%u) state=%d bytes=[%02x %02x %02x %02x]\n",
		      id_, status, status, (int)state_,
		      (unsigned)(status & 0xff), (unsigned)((status >> 8) & 0xff),
		      (unsigned)((status >> 16) & 0xff), (unsigned)((status >> 24) & 0xff));
		if (state_ == State::Handshaking) {
			debug("proc %d: got handshake reply\n", id_);
			// Do NOT reset total_failures_ here. The child handshakes
			// before pthread_create, so handshake success doesn't prove
			// the child can execute. Only reset after full execution.
			ChangeState(State::Idle);
			Execute();
		} else if (state_ == State::Executing) {
			debug("proc %d: got execute reply\n", id_);
			// Child completed full execution cycle — reset counter.
			total_failures_ = 0;
	...
	bool ReadOutput()
	{
		...
		static constexpr size_t kMaxOutput = 16 << 20; // 16 MiB
		const size_t kChunk = 1024;
		if (output_.size() >= kMaxOutput) {
			// Drain and discard to keep the pipe flowing.
			char discard[kChunk];
			read(stdout_pipe_, discard, kChunk);
			return true;
		}
...
public:
	Runner(Connection& conn, int vm_index, const char* bin)
	    : conn_(conn),
	      vm_index_(vm_index)
	{
		// Install new_handler to diagnose bad_alloc with context info.
		// Prints diagnostics (context, memory stats) and aborts via failmsg.
		instance_ = this;
...
private:
...
#if SYZ_RPC_DIAG
	// Ring buffer of recently executed request IDs for correlation with corruption events.
	static constexpr size_t kRecentReqCount = 16;
	int64_t recent_req_ids_[kRecentReqCount] = {};
	size_t recent_req_count_ = 0;
#endif
	...
	void Loop()
	{
		...
		// Reap any abandoned children (stuck in D-state) that have
		// since exited. Prevents zombie accumulation that exhausts
		// the system process table and causes pthread_create failures.
		Subprocess::ReapAbandoned();

		if (select.Ready(conn_.FD())) {
			alloc_context_ = "Recv(HostMessage)";
			rpc::HostMessageRawT raw;
			if (!conn_.Recv(raw)) {
				// Corrupt flatbuffer — bad_alloc was caught via
				// setjmp/longjmp. Skip this message and continue.
				debug("skipping corrupt host message\n");
#if SYZ_RPC_DIAG
				DumpDiagnosticsAndExit();
				// not reached
#endif
				alloc_context_ = "Loop";
				return;
			...
			if (auto* msg = raw.msg.AsExecRequest()) {
				alloc_context_ = "Handle(ExecRequest)";
				Handle(*msg);
			} else if (auto* msg = raw.msg.AsSignalUpdate()) {
				alloc_context_ = "Handle(SignalUpdate)";
				Handle(*msg);
...
#if SYZ_RPC_DIAG
	// Full diagnostic dump on first corruption, then clean exit.
	// The manager sees "lost connection" and restarts the instance fresh.
	// This is preferable to continuing in a degraded kernel state where
	// corruption frequency accelerates (observed pattern: 01:12, 01:40, 01:54).
	void DumpDiagnosticsAndExit()
	{
		fprintf(stderr, "\n========================================\n");
		fprintf(stderr, "=== SYZ_RPC_DIAG: CORRUPTION DETECTED ===\n");
		fprintf(stderr, "========================================\n\n");

		// Connection-level diagnostics (message counter, CRC, socket stats).
		conn_.DumpRecvDiagnostics();
		...
		fprintf(stderr, "RLIMIT_AS: cur=%llu max=%llu\n",
			(unsigned long long)rl_as.rlim_cur,
			(unsigned long long)rl_as.rlim_max);
		fprintf(stderr, "RLIMIT_DATA: cur=%llu max=%llu\n",
			(unsigned long long)rl_data.rlim_cur,
			(unsigned long long)rl_data.rlim_max);
		fprintf(stderr, "maxrss_kb: %ld\n", ru.ru_maxrss);

		// File descriptor scan.
		fprintf(stderr, "\n--- File Descriptors (0-%d) ---\n", kFdLimit - 1);
		int open_count = 0;
		for (int i = 0; i < kFdLimit; i++) {
			int flags = fcntl(i, F_GETFD);
			if (flags != -1) {
				int fl = fcntl(i, F_GETFL);
				// Identify fd type via fstat.
				struct stat st;
				...
				fprintf(stderr, "  fd=%d type=%s flags=0x%x\n", i, type, fl);
				open_count++;
			}
		}
		fprintf(stderr, "total_open_fds: %d\n", open_count);
		...
		// Abandoned children.
		fprintf(stderr, "abandoned_children: %zu\n", Subprocess::AbandonedCount());

		// Recently executed request IDs.
		fprintf(stderr, "\n--- Recent Request IDs (last %zu) ---\n", kRecentReqCount);
		size_t count = std::min<size_t>(recent_req_count_, kRecentReqCount);
		for (size_t i = 0; i < count; i++) {
			size_t idx = (recent_req_count_ - count + i) % kRecentReqCount;
			fprintf(stderr, "  [%zu]: req_id=%lld\n", i, (long long)recent_req_ids_[idx]);
		}

		fprintf(stderr, "\n========================================\n");
		fprintf(stderr, "=== BAILING OUT — clean exit for manager restart ===\n");
		fprintf(stderr, "========================================\n\n");

		// Clean exit — NOT failmsg/SYZFAIL (that would trigger crash reproduction).
		// Manager sees "lost connection" and restarts the instance.
		doexit(0);
	}
#endif

	static inline Runner* instance_ = nullptr;
	static inline const char* alloc_context_ = "unknown";

	static void BadAllocHandler()
	{
		// If we're in Recv::UnPackTo, recover via longjmp instead of aborting.
		// This allows the runner to skip the corrupt message and continue
		// fuzzing. The corrupt flatbuffer passed the Verifier but has bogus
		// vector lengths that trigger multi-GB allocations during unpack.
		// Small memory leak from partially-constructed objects is acceptable.
		if (recv_unpack_active_) {
			recv_unpack_active_ = false;
			longjmp(recv_unpack_jmpbuf_, 1);
		}

		// Not in UnPackTo — can't recover, abort with full diagnostics.
		struct rlimit rl_as = {}, rl_data = {};
		getrlimit(RLIMIT_AS, &rl_as);
		getrlimit(RLIMIT_DATA, &rl_data);
		struct rusage ru = {};
		...
		size_t queued_data = 0;
		if (instance_) {
			for (const auto& proc : instance_->procs_)
				total_output += proc->OutputSize();
			queued_requests = instance_->requests_.size();
			for (const auto& req : instance_->requests_)
				queued_data += req.data.size();
		}

		failmsg("bad_alloc",
			"context=%s procs=%zu queued_requests=%zu queued_data=%zu total_output=%zu "
			"rlimit_as_cur=%llu rlimit_as_max=%llu "
			"rlimit_data_cur=%llu rlimit_data_max=%llu "
			"maxrss_kb=%ld",
			alloc_context_,
			instance_ ? instance_->procs_.size() : 0,
			queued_requests, queued_data, total_output,
			(unsigned long long)rl_as.rlim_cur,
			(unsigned long long)rl_as.rlim_max,
			(unsigned long long)rl_data.rlim_cur,
			(unsigned long long)rl_data.rlim_max,
	...
	int Handshake()
	{
		alloc_context_ = "Handshake";
		// Handshake stage 0: get a cookie from the manager.
		rpc::ConnectHelloRawT conn_hello;
		if (!conn_.Recv(conn_hello))
			fail("corrupt data during handshake (ConnectHello)");
		...
		if (!conn_.Recv(conn_reply))
			fail("corrupt data during handshake (ConnectReply)");
		if (conn_reply.debug)
			flag_debug = true;
		...
		if (!conn_.Recv(info_reply))
			fail("corrupt data during handshake (InfoReply)");
	...
	void Handle(rpc::ExecRequestRawT& msg)
	{
		debug("recv exec request %llu: type=%llu flags=0x%llx env=0x%llx exec=0x%llx size=%zu\n",
		      ...
		      msg.data.size());
#if SYZ_RPC_DIAG
		recent_req_ids_[recent_req_count_ % kRecentReqCount] = msg.id;
		recent_req_count_++;
#endif
...
static void runner(char** argv, int argc)
{
	...
	// Diagnostic: print inherited memory limits at startup.
	// Use fprintf (not debug()) because flag_debug is not set until after Handshake.
	{
		struct rlimit rl_as = {}, rl_data = {};
		getrlimit(RLIMIT_AS, &rl_as);
		getrlimit(RLIMIT_DATA, &rl_data);
		struct rusage ru = {};
		getrusage(RUSAGE_SELF, &ru);
		fprintf(stderr, "runner memory limits at startup: "
			"RLIMIT_AS cur=%llu max=%llu "
			"RLIMIT_DATA cur=%llu max=%llu "
			"maxrss_kb=%ld\n",
			(unsigned long long)rl_as.rlim_cur,
			(unsigned long long)rl_as.rlim_max,
			(unsigned long long)rl_data.rlim_cur,
	...
	// Fuzzer-generated programs can send arbitrary signals to the parent via
	// kill(getppid(), sig), including real-time signals (65-126 on FreeBSD).
	// Note: can't use NSIG here — on FreeBSD NSIG=32 (legacy), but the kernel
	// supports signals up to _SIG_MAXSIG=128. On Linux NSIG=64.
	// Use 128 to cover all platforms. signal() returns SIG_ERR for invalid
	// numbers, which is harmless since we don't check the return value here.
	...
	// Remove stale shmem files from a previous crashed/killed run.
	cleanup_stale_shmem("/xyz", "syz.");
